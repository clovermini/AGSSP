...
class MultiView(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        """
        #print('multi-view results ', results['img'].shape, ' --- gt_seg_map ', results['gt_seg_map'].shape)
        ...
        for trans in self.transforms:
            ...
            if idx == 0 and 'gt_seg_map' in results:
                results['gt_seg_map'] = outputs['gt_seg_map']
            idx += 1

            multi_views_outputs['img'].append(outputs['img'])
        results.update(multi_views_outputs)
        #print('multi-view results after ', results['img'][0].shape, ' // ', results['img'][1].shape, ' --- gt_seg_map ', results['gt_seg_map'].shape)
        return results
