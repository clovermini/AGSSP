...
class CustomDistillDataset(BaseDataset):
    """A generic dataset for multiple tasks.  support anomaly map for distill

    The dataset supports two kinds of style.

    Please use an annotation file to specify all samples, and each line indicates a sample:

        The annotation file (for ``with_label=True``, supervised tasks.): ::

           folder_1/xxx.png 0 anomaly_map1.npy
           folder_1/xxy.png 1 anomaly_map2.npy
           123.png 4 anomaly_map3.npy
           nsdf3.png 3 anomaly_map4.npy
           ...
    ...
    def load_data_list(self):
        ...
        if not self.ann_file:
            ...
            samples = [x.strip().rsplit(' ', 2) for x in lines]
        else:
        ...
        for sample in samples:
            if self.with_label:
                filename, gt_label, seg_map_path = sample
                img_path = backend.join_path(self.img_prefix, filename)
                info = {'img_path': img_path, 'gt_label': int(gt_label), 'seg_map_path':seg_map_path, 'seg_fields':[]}
            else:
