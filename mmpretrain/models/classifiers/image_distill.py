...
class ImageDistillClassifier(BaseClassifier):
    ...
    Args:
        ...
        distill_head (dict, optional): The head module to do distill from anomaly map or features. 
            See :mod:`mmpretrain.models.heads`.
            Notice that if the head is not set, almost all methods cannot be
            used except :meth:`extract_feat`. Defaults to None.
    ...
    def __init__(self,
                 ...
                 distill_head: Optional[dict] = None,
        ...
        super(ImageDistillClassifier, self).__init__(
        ...
        if distill_head is not None and not isinstance(distill_head, nn.Module):
            distill_head = MODELS.build(distill_head)

        self.backbone = backbone
        self.neck = neck
        self.head = head
        self.distill_head = distill_head
    ...
    def forward(self,
                inputs: torch.Tensor,
                data_samples: Optional[List[DataAnomalySample]] = None,
                mode: str = 'tensor'):
        ...
        - "predict": Forward and return the predictions, which are fully
          processed to a list of :obj:`DataAnomalySample`.
        ...
        Args:
            inputs (torch.Tensor): The input tensor with shape
                (N, C, ...) in general.
            data_samples (List[DataAnomalySample], optional): The annotation
        ...
        Returns:
            ...
            - If ``mode="predict"``, return a list of
              :obj:`mmpretrain.structures.DataAnomalySample`.
    ...
    def extract_feat(self, inputs, stage='neck'):
        ...
        Examples:
            ...
            torch.Size([1, 3072])
        """  # noqa: E501
        assert stage in ['backbone', 'neck', 'backbone_neck', 'pre_logits'], \
            (f'Invalid output stage "{stage}", please choose from "backbone", '
             '"neck" and "pre_logits"')

        x = self.backbone(inputs)   # [bs, 1024, 7, 7]
        #print('infer model backbone x ', len(x), ' ', x[0].size())  
        if stage == 'backbone':
            return x

        if self.with_neck:
            x_neck = self.neck(x)    # [bs, 1024]
            #print('infer model  neck x ', len(x), ' ', x[0].size())
            if stage == 'backbone_neck':
                return (x, x_neck)
            x = x_neck
    ...
    def loss(self, inputs: torch.Tensor,
             data_samples: List[DataAnomalySample]) -> dict:
        """Calculate losses from a batch of inputs and data samples.

        Args:
            inputs (torch.Tensor): The input tensor with shape
                (N, C, ...) in general.
            data_samples (List[DataAnomalySample]): The annotation data of
                every samples.

        Returns:
            dict[str, Tensor]: a dictionary of loss components
        """
        feats_back, feats_neck = self.extract_feat(inputs, stage='backbone_neck')
        
        losses = dict()
        if self.head is not None:
            loss_cls = self.head.loss(feats_neck, data_samples)
            losses.update(add_prefix(loss_cls, 'cls'))
        #print('feats_neck ', feats_neck[0].shape, ' loss_cls ', loss_cls)

        loss_distill = self.distill_head.loss(feats_back, data_samples)
        losses.update(add_prefix(loss_distill, 'distill'))
        #print('feats_back ', feats_back[0].shape, ' loss_distill ', loss_distill)
