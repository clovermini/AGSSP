...
@MODELS.register_module()
class MoCoV3_Distill(BaseSelfSupervisor):
    """MoCo v3 with distill from anomaly map.
    ...
    def loss(self, inputs: List[torch.Tensor], data_samples: List[DataSample],
        ...
        # compute query features, [N, C] each
        back_view1 = self.backbone(view_1)
        #print('back_view1 ', len(back_view1), ' -- ', back_view1[0].size())
        q1 = self.neck(back_view1)[0]
        q2 = self.neck(self.backbone(view_2))[0]
        ...
        losses = dict()
        losses['mocov3.loss'] = loss
        
        loss_distill = self.distill_head.loss(back_view1, data_samples)
