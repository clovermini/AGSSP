...
class SimMIM(BaseSelfSupervisor):
    ...
    def loss(self, inputs: torch.Tensor, data_samples: List[DataSample],
        ...
        img_latent = self.backbone(inputs, mask)  # [64, 1024, 6, 6]
        #print('img_latent ', len(img_latent), ' ', img_latent[0].shape)
        img_rec = self.neck(img_latent[-1])   # [64, 192, 192]
        #print('img_rec ', img_rec[0].shape)
...
@MODELS.register_module()
class SimMIM_Distill(BaseSelfSupervisor):
    """SimMIM with distill from anomaly map.
    ...
    def loss(self, inputs: torch.Tensor, data_samples: List[DataSample],
        ...
        losses = dict()
        loss = self.head.loss(img_rec, inputs, mask)
        losses['rec.loss'] = loss
        
        img_feat = self.extract_feat(inputs)
        loss_distill = self.distill_head.loss(img_feat, data_samples)
        losses.update(add_prefix(loss_distill, 'distill'))

        #print('loss ', loss, ' loss_distill ', loss_distill)
