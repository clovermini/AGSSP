    ...
    def detection_evaluate(self, dataset, results, topk=20, eval_fn=None):
        ...
        for i, (result, ) in enumerate(zip(results)):
            ...
            for idx in range(len(pred_scores)):
                if float(pred_scores[idx]) >= 0.3:
                    pred_bboxes_filter.append(pred_bboxes[idx])
                    pred_scores_filter.append(pred_scores[idx])
                    pred_labels_filter.append(pred_labels[idx])
            pred_bboxes = np.array(pred_bboxes_filter)
            pred_scores = np.array(pred_scores_filter)
            pred_labels = np.array(pred_labels_filter)
            #if '1677092371372' in data_info['img_path']:
            #    print('data_info ', data_info)
            #    print('pred_scores_filter ', pred_scores)

            if len(data_info['gt_bboxes_labels']) == 0 and len(pred_labels_filter) == 0: # 都是背景
                mAP = -1
                continue
            elif len(data_info['gt_bboxes_labels']) > 0 and len(pred_labels_filter) == 0:
                mAP = 0.3
            #elif len(data_info['gt_bboxes_labels']) == 0 and len(pred_labels_filter) > 0:
            #    mAP = -max(pred_scores_filter)
            else:
                dets = []
                for label in range(len(dataset.metainfo['classes'])):
                    ...
                    try:
                        ...
                        print('index ', index, ' data_info ', data_info['img_path'], ' -- ', data_info['gt_bboxes_labels'])
                        print('pred_bboxes ', pred_bboxes[index])
                        print('pred_scores ', pred_scores[index].reshape((-1, 1)))
                        pred_bbox_scores = np.array([], dtype=np.float32)

                    dets.append(pred_bbox_scores)
                    #print('dets ', dets)
                mAP = eval_fn(dets, data_info)
            #print(mAP, ' gt ', len(data_info['gt_bboxes_labels']), ' -- pred ', len(pred_labels_filter))
            _mAPs[i] = mAP
            #print('_mAPs ', _mAPs)
            prog_bar.update()
