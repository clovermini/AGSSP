...
Mvtec_CLSNAMES = ['carpet', 'bottle', 'hazelnut', 'leather', 'cable', 'capsule', 'grid', 'pill',
                    'transistor', 'metal_nut', 'screw', 'toothbrush', 'zipper', 'tile', 'wood']

Mvtec_CLSNAMES_map_index = {}
for k, index in zip(Mvtec_CLSNAMES, range(len(Mvtec_CLSNAMES))):
	Mvtec_CLSNAMES_map_index[k] = index
...
class MVTecDataset(data.Dataset):
	...
	def __getitem__(self, index):
		...
		img_mask = [] if img_mask is None else img_mask
		return {'img': img, 'img_mask': img_mask, 'cls_name': cls_name, 'anomaly': anomaly,
				'img_path': os.path.join(self.root, img_path), "cls_id":Mvtec_CLSNAMES_map_index[cls_name]}
